...
""" This is the main function from where the process starts. This function yields the source data for every target data calling KNN function.

Need to run individually for every target data by  setting the valid range in the for loop such that 
it will execute only once  for every target data. 
...
Similarly, setting the for loop range as (1,2) will make it run for "ant2" target project and so on.
Execute all the code cells below to apply the feature selection phase and calculate the NEPMs and EPMs for that target project.
...
for i in range(0,1):
    train_data,X_test,y_test=KNN(list1[i],i,['ant2','ant3','ant4','ant5'])
...
# create a dataframe for storing the selected features
import pandas as pd
df_selected_feature=pd.DataFrame()
result=[]
...
def f_per_particle(m):
    ...
    if np.count_nonzero(m) == 0:
        ...
        X_test=target_test # target_test is the 20% of the target data used in the feature selection phase
    ...
    prec=precision_score(target_test_labels,y_pred)
    ...
    mcc=matthews_corrcoef(target_test_labels,y_pred)
...
"""
Here, BPSO is called 10 times, in every iteration, it is randomly selecting the features from the source data  retreived after applying smote
and validating its performance on 20% of target data. After we obtain the imporatnt features, the source and the target data is modified
to have these selected features only. Finally, a NB model is constructed with modified source data and is validated on rest 80% of
...
# Perform optimization
  ...
  # now checking with selected features
    classifier = GaussianNB()

    X_train_selected_features=new_source.iloc[:,pos==1]

  # checking on 80% of test data
...
# calculation of NEPMs
    ...
    prec1=precision_score(target_train_labels,y_pred2)
...
# calculation of EPMs
    ...
    PII2=PII_20(target2)  
    costeffort2=costeffort20(target2)  
    IFA2=IFA20(target2)
    r=[rec1,prec1,g_measure,mcc,pf,PII2,costeffort2,IFA2]
    result.append(r)
